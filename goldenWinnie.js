setInterval(() => {
    if(DeBread.randomNum(1, save.goldenWinnieChance) === 1 && save.goldenWinniesAvailable) {
        createGoldenWinnie(DeBread.randomNum(0, 4))
...
function startGoldenWinnieEvent(type) {
    ...
    if(type === 3) {
        for(let i = 0; i < 2; i++) {
            createGoldenWinnie(DeBread.getRandomInArray([0, 1, 2, 4]))
        }
